...
def test_agent_report_schedule_can_be_created_then_disabled(
    ...
    expect(agent.get_by_role("heading", name="Repair canary gate")).to_be_visible()
    expect(agent.get_by_text("get_iplayer", exact=True)).to_be_visible()
    expect(agent.get_by_text("autonomous locked", exact=True)).to_be_visible()
