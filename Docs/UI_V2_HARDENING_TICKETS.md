...
| CAT-003 | Merge all allowed top-level compose sections | P2 | backend | STK-003 | Complete |
| STK-004 | `up -d --remove-orphans` (after review) | P2 | backend | — | Complete |
| STK-005 | Detect duplicate compose filenames | P2 | backend | — | Complete |
| STK-006 | Cache one Docker snapshot for status polling | P2 | backend+ui | — | Complete |
...
| UI-004 | Mobile drawer: focus trap, inert bg, skip link | P2 | frontend | — | Complete |
| UI-005 | Derive service-link scheme (no hardcoded http) | P2 | frontend | — | Complete |
...
| CFG-001 | Relocate runtime state out of the source checkout | P2 | backend | — | Complete |
| ARCH-001 | Split oversized modules during the above fixes | P2 | both | — | Complete |
...
- **CAT-003** merge all allowed top-level compose sections (configs/secrets) — `catalog_manager.py:431-443`. **Complete 2026-06-28:** catalog installs now merge the complete bounded set of Compose resource sections (`networks`, `volumes`, `configs`, and `secrets`) with template substitution and STK-003 round-trip formatting preservation. Identical existing definitions are reused without replacing user-owned comments; differing definitions fail with a structured `409`, malformed catalog sections fail with `400`, and malformed existing sections fail with `409`. All resource sections are validated before backup or service mutation, so rejected installs leave the Compose file byte-for-byte unchanged and create no backup. Focused Compose + catalog coverage `47 passed`. Full `tox -e all`: Ruff clean; unit `671 passed, 1 skipped`; E2E `172 passed, 26 skipped`.
- **STK-004** add `--remove-orphans` after reviewing external-service preservation — `stack_manager.py:237-268,748-786`. **Complete 2026-06-28:** every stack-level Compose `up` now reconciles the canonical stack file with `--remove-orphans`, covering synchronous API calls, catalog installs, scheduled updates, backup restarts, and streamed operations through one shared argument builder. Cleanup remains limited to containers for services omitted from the same Compose project; separately named Compose projects and external networks/volumes are not removed. LimeOS therefore treats same-project containers missing from the canonical stack file as stale, while intentionally separate workloads must retain a distinct project identity. `down`, `restart`, `pull`, `start`, and service-scoped `stop` remain unchanged. Focused stack coverage `70 passed`. Full `tox -e all`: Ruff clean; unit `677 passed, 1 skipped`; E2E `172 passed, 26 skipped`.
- **STK-005** detect duplicate compose filenames; block until resolved — `stack_manager.py:20-60`. **Complete 2026-06-28:** Compose discovery now enumerates all four supported filenames and raises a typed `compose_file_conflict` instead of silently choosing by priority when more than one exists. Conflicted stacks remain visible in list and scan responses with deterministic filename details and `status: conflict`, while status probes are suppressed. Compose reads, saves, restores, backups, logs, lifecycle commands, stack deletion, operation creation, and catalog install/removal fail closed with a structured `409`; rejected writes create no backup and leave every candidate file unchanged. Focused stack + catalog coverage `119 passed`. Full `tox -e all`: Ruff clean; unit `683 passed, 1 skipped`; E2E `172 passed, 26 skipped`.
- **STK-006** cache one Docker snapshot for status; prevent overlapping polls — `stack_manager.py:296-305`, `stacks-page.tsx`. **Complete 2026-06-28:** `/api/stacks?status=true` now runs one request-scoped `docker ps -a` snapshot instead of one sequential `docker compose ps` process per stack. Containers are mapped through Compose `config_files` labels, including multi-file projects, with a working-directory fallback; counts and `running`/`partial`/`stopped` summaries are then computed in memory. Snapshot timeout, execution, or nonzero-exit failures mark every eligible stack `unknown` with scoped error detail instead of reporting a false stopped state. Conflict-only lists spawn no Docker process. Both v2 and legacy stack pages suppress interval/manual/action loads while another list request is in flight, preventing overlapping polls. Focused stack + catalog coverage `121 passed`; legacy JavaScript syntax, TypeScript, and production build passed (`100.57 kB` initial JS gzip). Full `tox -e all`: Ruff clean; unit `685 passed, 1 skipped`; E2E `172 passed, 26 skipped`.
...
- **UI-004** mobile drawer focus trap / inert background / overscroll / skip-link — `app-shell.tsx`. **Complete 2026-06-28:** the mobile navigation now behaves as a modal dialog: focus enters on the close control, Tab/Shift+Tab remain contained, Escape and backdrop activation close it, and focus returns to the menu trigger after native background inertness is removed. The app background is marked `inert` and hidden from assistive technology while open; body and root scrolling/overscroll are locked and restored exactly on close. Crossing the desktop breakpoint closes an open drawer. A keyboard-visible skip link now targets the programmatically focusable main region. Browser coverage verifies skip-link focus, drawer entry/trapping/restoration, inert background, and scroll-lock cleanup at phone width; the complete foundation viewport/UI-mode matrix passed (`34 passed, 6 skipped`). TypeScript and production build passed (`101.13 kB` initial JS gzip). Full `tox -e all`: Ruff clean; unit `685 passed, 1 skipped`; E2E `173 passed, 26 skipped`.
- **UI-005** derive service-link scheme from metadata, not hardcoded `http://` — `dashboard-home.tsx`, `containers-page.tsx`. **Complete 2026-06-28:** container responses now carry validated service-link metadata from an explicit `limeos.web.url`, `limeos.web.scheme`, or deployment-wide `PIHEALTH_SERVICE_LINK_SCHEME`, in that precedence order. Only absolute credential-free HTTP(S) URLs and the `http`/`https` schemes are accepted. One shared frontend builder handles explicit reverse-proxy URLs, published-port URLs, and IPv6 host formatting for both dashboard and container views; missing or unsafe metadata hides the external link instead of guessing a protocol. Backend container/helper coverage `87 passed`; explicit HTTPS behavior passed on dashboard and containers across desktop, phone, and tablet (`6 passed`). TypeScript and production build passed (`101.22 kB` initial JS gzip). Configuration and fail-closed behavior are documented in the user guide. Full `tox -e all`: Ruff clean; unit `692 passed, 1 skipped`; E2E `173 passed, 26 skipped`.
...
- **CFG-001** relocate runtime state: config `/etc/limeos`, state `/var/lib/limeos`, logs `/var/log/limeos`, secrets in credential storage (+ migration plan) — `app.py:29-39`, `catalog_manager.py:20-25`, `backup_scheduler.py:24-25`. **Complete 2026-06-28:** one environment-overridable runtime path contract now separates operator configuration, mutable scheduler/plugin state, logs, credentials, and checkout-owned schemas/catalog assets. All backend consumers, backup sources, helper allowlists, systemd setup, and user documentation use the new locations. Installation performs an atomic, idempotent copy migration from checkout-local files and `/etc/pi-health.env`; existing destinations are never overwritten and legacy files remain available for rollback. Migration tests cover routing, permissions, idempotency, and no-overwrite behavior. The E2E launcher uses isolated runtime roots, exposing and removing its prior dependency on checkout-local auto-update state. Ruff and Bash syntax checks passed; focused runtime coverage `327 passed, 1 skipped`; full unit coverage `695 passed, 1 skipped`; full E2E `172 passed, 27 skipped`.
- **ARCH-001** split oversized modules (`app.py` ~1699 lines; containers/stacks/storage pages >800) **as part of** the above fixes, not a standalone rewrite. **Complete 2026-06-29:** backend system telemetry and pure container metadata/statistics/topology calculations now live in `system_stats.py` and `container_helpers.py`; `app.py` remains the Flask/application-state boundary and preserves its existing patchable exports. The v2 container list, stack card, and storage plugin card now live under domain-specific component directories, while page modules retain polling, request orchestration, and modal workflow state. Routes, API contracts, DOM selectors, responsive behavior, and action semantics are unchanged. Focused backend coverage `155 passed, 1 skipped`; affected containers/stacks/storage Playwright parity `29 passed`; TypeScript and production build passed (`101.13 kB` initial JS gzip). Full `tox -e all`: Ruff clean; unit `695 passed, 1 skipped`; E2E `172 passed, 27 skipped`.
