...
app = FastAPI(title="Radarr MCP Server", version="0.1.0")
register_exception_handler(app)


def _client() -> ArrClient:
    return ArrClient(
        base_env="RADARR_BASE_URL",
        default_base="http://radarr:7878/api/v3",
        api_key_env="RADARR_API_KEY",
        timeout_env="RADARR_HTTP_TIMEOUT",
    )
...
class MovieSearchRequest(BaseModel):
    movie_id: int = Field(..., ge=1, alias="movieId")
...
@app.post("/tools/search_movie")
def search_movie(request: MovieSearchRequest) -> Dict[str, Any]:
    client = _client()
    payload = {"name": "MoviesSearch", "movieIds": [request.movie_id]}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Radarr MCP Service")
