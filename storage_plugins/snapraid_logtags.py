...
def parse_log_tags(text: str) -> LogTagParseResult:
    ...
    for raw_line in text.splitlines():
        ...
        event = parse_log_tag_line(line)
        if not event:
            continue
        apply_tag_event(result, event)

    return result


def parse_log_tag_line(line: str) -> dict[str, Any] | None:
    if not line or ":" not in line:
        return None
    parts = line.split(":")
    name = parts[0]
...
def apply_tag_event(result: LogTagParseResult, event: dict[str, Any]) -> None:
    name = event.get("name")
